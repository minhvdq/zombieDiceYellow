public class ZombieDice {
        ...
        for( int i  = 0; i < MAX_SCORE; i ++){
            for( int j = 0; j < NUM_DICE; j ++){
                if( i == 0){
                    ...
                    pExceed[i][j] = computepExceed( i, j, 1);
                }
            }
        }
                    ...
                    for( int opScore = 0; opScore < MAX_SCORE; opScore ++){
                        for( int turnTotal = 0; turnTotal < MAX_SCORE; turnTotal ++ ){
                            for( int  sg = 0; sg < NUM_DICE; sg  ++){
                                ...
                                double pHold = 1 - computeProbWin( 1 - curPlayer, opScore, curScore + turnTotal, 0, 0);
                                if( curPlayer == 1 && curScore + turnTotal > opScore && curScore + turnTotal >= WIN_SCORE){
                                    pHold = 1.0;
                                }
                                if( turnTotal == 0 || (curPlayer == 1 && curScore + turnTotal < opScore && opScore >= WIN_SCORE) ){
                                    pHold = 0.0;
                                }
                                double pRoll = 0.0;
                                ...
                                // else{
                                    for( int newSg = 0; newSg < NUM_DICE - sg; newSg ++){
                                        for( int newB = 0 ; newB <= NUM_DICE - newSg; newB ++){
                                            int f = 3 - newB - newSg;
                                            pRoll += pDiceRoll[NUM_DICE][newB][newSg] * computeProbWin( curPlayer, curScore, opScore, turnTotal + newB, sg + newSg);
                                        }
                                    }
                                    for( int newSg = NUM_DICE - sg; newSg <= NUM_DICE; newSg ++){
                                        for( int newB = 0 ; newB <= NUM_DICE - newSg; newB ++){
                                            int f = 3 - newB - newSg;
                                            pRoll += pDiceRoll[NUM_DICE][newB][newSg] * (1 - computeProbWin( 1 - curPlayer, opScore, curScore, 0, 0) );
                                        }
                                    }
                                // }
    ...
    public double computeProbWin( int curPlayer, int curScore, int opScore, int turnTotal, int sg ){
        if( curPlayer == 0 && curScore >= WIN_SCORE){
            if( curScore > opScore ){
                return 1.0;
            }
            if( curScore < opScore ){
